...
class FacialDetector:
    ...
    def image_pyramid(self, image, original_size, min_size):
        """
        Aceasta functie retuneaza o "piramida a imaginii", o reprezentare pe mai multe scale a unei imagini
        """
        yield image

        # new_height, new_width = int(1.5 * image.shape[0]), int(1.5 * image.shape[1])
        ...
        while True and failsafe<1000:
            ...
            # daca imaginea redimensionata nu corespunde dimensiunii furnizate, atunci se opreste constructia piramidei
            if new_width < min_size or new_height < min_size:
                break

            # if new_height == original_size[0] and new_width == original_size[1]:
            #     continue
    ...
    def run(self, return_descriptors=False):
        ...
        Directorul cu numele self.params.dir_test_examples contine imagini ce pot sau nu contine fete. Aceasta functie ar
        trebui sa detecteze fete atat pe setul de date MIT+CMU dar si pentru alte imagini (imaginile realizate cu voi la curs+laborator).
        Functia 'non_maximum_suppression' suprimeaza detectii care se suprapun (protocolul de evaluare considera o detectie duplicata
        ca fiind falsa). Suprimarea non-maximelor se realizeaza pentru fiecare imagine.
        :return:
        ...
        file_names = np.array([])  # pentru fiecare detectie se va salva numele fisierului asociat
        ...
        for i in range(num_test_images):
            ...
            iterate_once = False
            iter=0
            for img_resised in self.image_pyramid(image=img, original_size=(L, C), min_size=self.params.dim_window):
                iter+=1
                #print(iter)
                # pentru antrenarea cu exemple puternic negative se for extrage doar descriptorii din imaginea cu dimensiunea initiala
                if iterate_once:
                    break
                if self.params.use_hard_mining and return_descriptors:
                    iterate_once = True
                ...
                # pentru imaginea de dimensiune LxC obtinem descriptorul HOG asociat imaginii
                ...
                # pentru cele l celule pe orizontala si c celule pe verticala, glisam fereastra ce contine kxk celule
                ...
                for y_min in range(l - k):
                    for x_min in range(c - k):
                        ...
                        df = hog_descriptor[y_min:y_max, x_min:x_max].copy().flatten()
                        if return_descriptors and not self.params.use_hard_mining:
                            if descriptors_to_return is not None:
                                descriptors_to_return = np.concatenate((descriptors_to_return, df), axis=0)
                            else:
                                descriptors_to_return = np.array([df])
                        ...
                        if score[0][0] > self.params.threshold:
                            if self.params.use_hard_mining and return_descriptors:
                                if current_image_descriptors is None:
                                    current_image_descriptors = np.reshape(df, (1, df.shape[0]))
                                else:
                                    df = np.reshape(df, (1, df.shape[0]))
                                    current_image_descriptors = np.concatenate((current_image_descriptors, df), axis=0)

                            current_image_scores = np.concatenate((current_image_scores, score), axis=None)
                            width_ratio, height_ratio = C / W, L / H
                            # determinam coltul stanga-sus in imaginea redimensionata
                            x0 = int(((x_min + 1) * self.params.dim_hog_cell) * width_ratio)
                            y0 = int(((y_min + 1) * self.params.dim_hog_cell) * height_ratio)
                            # determinam coltul dreapta-jos in imaginea redimensionata
                            ...
                            if current_image_detections is None:
                                ...
                                current_image_detections = np.concatenate(
                                    (current_image_detections, np.array([[x0, y0, x1, y1]])), axis=0)
